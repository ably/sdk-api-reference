...
Interact with a specific channel instance. A channel instance is created or returned by [`Channels GET()`]{@link}.
...
|| direction: .Backwards \| .Forwards api-default .Backwards, | RSL2b2 | The order for which messages are returned in. The default is `Backwards` which orders messages from most recent to oldest. |
|| limit: int api-default 100 | RSL2b3 | An upper limit on the number of messages returned. The default is 100. |
...
| setOptions() => io || RSL7 | Set the [`ChannelOptions`]{@link} for the channel, such as encryption-related parameters. |
